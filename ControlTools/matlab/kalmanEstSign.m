...
Q_estimate1 = Q;
QuailAccel_noise_mag = 0.5; %process noise: the variability in how fast the Quail is speeding up (stdv of acceleration: meters/sec^2)
NinjaVision_noise_mag = 15;  %measurement noise: How mask-blinded is the Ninja (stdv of location, in meters)
...
%initize estimation variables
Q_loc_estimate = []; %  Quail position estimate
vel_estimate1 = []; % Quail velocity estimate
Q= [0; 0]; % re-initized state
P_estimate1 = P;
P_mag_estimate = [];
predic_state = [];
...
for t = 1:length(Q_loc)
    ...
    %Store for plotting
    Q_loc_estimate = [Q_loc_estimate; Q_estimate(1)];
    vel_estimate1 = [vel_estimate1; Q_estimate(2)];
    P_mag_estimate = [P_mag_estimate; P(1)]
end
hold on
% Plot the results
